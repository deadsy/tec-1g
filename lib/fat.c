//------------------------------------------------------------------------------
// See: https://github.com/strawberryhacker/fat32
...
// 4-bit Lookup Table
static const uint8_t lut_4bit[16] = {
	4, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0
};

static uint8_t ctz8(uint8_t val) {
	if (val == 0)
		return 8;
	// Check lower nybble
	uint8_t nybble = val & 0x0F;
	if (nybble != 0) {
		return lut_4bit[nybble];
	}
	// Fall back to upper nibble
	return 4 + lut_4bit[val >> 4];
}
...
static uint32_t sfn_cluster(Sfn *sfn) {
	return (uint32_t) sfn->clust_hi << 16 | sfn->clust_lo;
}
...
static bool dir_at_root(Dir *dir) {
	return dir->clust == dir->fat->root_clust && dir->sect == clust_to_sect(dir->fat, dir->fat->root_clust) && dir->idx == 0;
}
...
int fat_mount(DiskOps *ops, int partition, Fat *fat, const char *name) {
	...
	fat->clust_shift = ctz8(bpb->sect_per_clust);
	...
	if (info->tail_sig != FSINFO_TAIL_SIG || info->head_sig != FSINFO_HEAD_SIG || info->struct_sig != FSINFO_STRUCT_SIG || info->next_free == 0xffffffff || info->free_cnt == 0xffffffff)
		return FAT_ERR_NOFAT;
